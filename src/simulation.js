...
export const SPRAY_COUNT=3584, ROCK_COUNT=105, TREE_COUNT=100;
export class IslandSimulation {
  static async create(quality, seed, progress=()=>{}, onError=console.error) {
    ...
    await sim.initialize(progress);return sim;
  }
  constructor(runtime,kernels,quality,seed=1741) {
    ...
    this.grid=gridFor(quality);this.n=this.grid.nx*this.grid.nz;this.count=quality.grass;
    ...
    buffer('Terrain',this.n*8*4);buffer('Trees',TREE_COUNT*4*4);buffer('Foliage',TREE_COUNT*96*4*8*4);
    buffer('Particles',SPRAY_COUNT*12*4);buffer('Spray',SPRAY_COUNT*8*4);
    buffer('Controls',new Float32Array([1,-15,0]));
  ...
  values(extra={}) {return {...this.grid,n:this.count,start:0,count:this.n,dt:1/120,enhanced:1,time:this.time,flow:this.settings.flow,closed:0,seed:this.seed,reset:0,season:this.settings.season,water:this.settings.moisture,...extra};}
  ...
  syncControls() {
    const value=[this.settings.strength,-15,this.settings.tide],key=value.join(',');
  ...
  async diagnostics() {
    ...
    return {grid:this.grid,simulatedTime:this.time,steps:this.steps,nonfinite,minDepth,maxDepth,volume,foam,wetCells,activeGrass,submergedRoots,draws:Array.from(commands),runtime:{...this.runtime.stats},adapter:this.runtime.describe()};
  }
  async closedTests() {
