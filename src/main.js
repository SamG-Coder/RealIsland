...
const controls=createCamera($('view'),key=>{if(key==='KeyH')photo();if(key==='KeyP')togglePause();const views={Digit1:'island',Digit2:'meadow',Digit3:'river',Digit4:'shore',Digit5:'estuary'};if(views[key])goto(views[key]);});
...
async function goto(name){const v=VIEWS[name];if(!v||!sim)return;let position=[...v.position];
 ...
 pointCamera(controls.camera,position,v.target);renderer.lastWeather=-Infinity;
...
async function diagnostics(){const prior=stopped;stopped=true;await sim.runtime.idle();const state=await sim.diagnostics(),tests=await sim.closedTests();const report={version:'RealIsland 0.1',date:new Date().toISOString(),state,tests,renderShaders:renderer.compilation,frameSamplesMs:[...frames],renderSize:[renderer.width,renderer.height],errors:[...errors],note:'Software adapters are valid for correctness, not hardware performance estimates.'};if(!prior){stopped=false;last=0;requestAnimationFrame(frame);}return report;}
function wire(){
 ...
 for(const name of ['flow','strength','wind','clouds','tide','exposure'])$(name).addEventListener('input',()=>{const v=Number($(name).value);if(name==='exposure')renderer.exposure=v;else sim.settings[name]=v;$(name+'-value').textContent=name==='clouds'?`${Math.round(v*100)}%`:name==='tide'?`${v.toFixed(2)} m`:name==='flow'?`${v.toFixed(1)}×`:v.toFixed(2);renderer.lastWeather=-Infinity;});
 ...
 $('diagnostics').onclick=async()=>{try{toast('Running explicit water and GPU checks…');const report=await diagnostics();download(report,'RealIsland-validation.json');toast(report.tests.every(x=>x.pass)&&!report.errors.length?'Validation report exported.':'Report exported: inspect the failed checks.');}catch(error){toast(error.message);}};
}
