...
export function installGameUI(){
 ...
 <section id="game-menu" class="game-overlay" aria-label="Game menu">
  ...
  <div id="game-settings-panel" class="game-actions" hidden>
   ...
   <p class="game-muted">Quality changes reload the island. Your current journey is saved first.</p><button id="game-fullscreen" class="mobile-only">Fullscreen · landscape</button><button id="game-settings-back">Back</button>
  </div>
 ...
 <div id="touch-move" class="touch-stick" role="group" aria-label="Movement joystick"><span class="stick-knob"></span><span class="stick-label">MOVE</span></div>
 <div id="touch-look" class="touch-stick" role="group" aria-label="Look joystick"><span class="stick-knob"></span><span class="stick-label">LOOK</span></div>
 <button id="touch-sprint" class="touch-action">Sprint</button><button id="touch-jump" class="touch-action">Jump ↑</button>
 <button id="touch-interact" class="touch-action">Interact</button><button id="touch-fullscreen" aria-label="Enter fullscreen">⛶</button>
 </div><div id="game-toast"  role="status" hidden></div></div>
 <section id="rotate-prompt" role="dialog" aria-label="Rotate to landscape"><div class="rotate-device">▯ ↻</div><h2>A little more room to explore</h2><p id="rotate-message">Turn your phone sideways for two-thumb controls.</p><button id="rotate-fullscreen">Enter landscape</button></section>`;
...
export class SurvivalGame {
 constructor(sim,renderer,camera,quality){
  ...
  this.wire();this.touch=new TouchControls(this);this.show('title');
 }
 ...
 show(phase){this.phase=phase;document.body.dataset.gamePhase=phase;this.touch?.reset();this.sim.playerInteraction=null;if(phase!=='playing'&&document.pointerLockElement)document.exitPointerLock();this.keys.clear();this.jumpQueued=false;this.dragging=false;this.accumulator=0;
 ...
 wire(){
  ...
  const canvas=$('view');
  canvas.addEventListener('click',e=>{if(e.pointerType==='touch'||this.touch?.enabled)return;if(this.phase==='playing')canvas.requestPointerLock?.()?.catch?.(()=>this.message('Mouse capture unavailable. Hold the mouse button to look.'));});
  canvas.addEventListener('pointerdown',e=>{if(e.pointerType==='touch'||this.touch?.enabled)return;if(this.phase!=='playing')return;this.dragging=true;this.mx=e.clientX;this.my=e.clientY;canvas.focus();});
  ...
  addEventListener('keydown',e=>{if(/INPUT|SELECT|TEXTAREA/.test(e.target.tagName))return;
   ...
   if(!e.repeat){if(e.code==='Space')this.jumpQueued=true;if(e.code==='KeyI')this.show('inventory');if(e.code==='KeyE')this.interact();}
 ...
 interact(){if(this.phase!=='playing')return;const action=interaction(this.state,this.world,this.resources);if(useInteraction(this.state,action)){this.message(action.kind==='drink'?'Water replenished':'Gathered 3 berries');this.updateActors();this.updateHUD();}}
 update(dt){
  if(this.phase!=='playing'||this.touch?.portrait){this.sim.playerInteraction=null;return cameraBasis(this.camera);}
  this.world.tide=this.sim.settings.tide;this.accumulator+=Math.min(dt,.1);
  this.yaw+=(this.touch?.look.x||0)*Math.min(dt,.1)*2.2*this.sensitivity;this.pitch=Math.max(-1.45,Math.min(1.45,this.pitch-(this.touch?.look.y||0)*Math.min(dt,.1)*1.8*this.sensitivity));
  const forward=-(this.touch?.move.y||0)+Number(this.keys.has('KeyW')||this.keys.has('ArrowUp'))-Number(this.keys.has('KeyS')||this.keys.has('ArrowDown')),side=(this.touch?.move.x||0)+Number(this.keys.has('KeyD')||this.keys.has('ArrowRight'))-Number(this.keys.has('KeyA')||this.keys.has('ArrowLeft'));
  const before={x:this.state.player.x,z:this.state.player.z};let advanced=0;
  while(this.accumulator>=1/60){this.motion=stepPlayer(this.state,this.world,{x:Math.sin(this.yaw)*forward+Math.cos(this.yaw)*side,z:-Math.cos(this.yaw)*forward+Math.sin(this.yaw)*side,sprint:this.touch?.sprint||this.keys.has('ShiftLeft')||this.keys.has('ShiftRight'),jump:this.jumpQueued},1/60);this.jumpQueued=false;this.accumulator-=1/60;advanced+=1/60;}
 ...
 updateHUD(){const s=this.state;for(const key of ['health','hunger','thirst','stamina']){$('need-'+key).value=s.player[key];$('need-'+key+'-value').textContent=Math.round(s.player[key]);}
  ...
  $('touch-interact').disabled=!action;$('touch-interact').textContent=action?.kind==='drink'?'Drink':action?.kind==='gather'?'Gather':'Interact';
  $('game-objective').textContent=!s.objectives.gather?'First supplies · gather berries from a nearby bush':!s.objectives.drink?(this.touch?.enabled?'Find freshwater · approach the river and tap Drink':'Find freshwater · approach the river and press E'):'Stay supplied · explore the island';
 }
}
