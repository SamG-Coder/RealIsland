...
export {cameraBasis};
export const VIEWS={
  island:{name:'Island overlook',position:[1000,670,1550],target:[0,70,-150]},
  lake:{name:'Headwater lake',position:[-275,0,-602],target:[-280,18,-690],groundOffset:5},
  river:{name:'River walk',position:[-55,0,-180],target:[-18,30,-310],groundOffset:2.1},
  meadow:{name:'Meadow',position:[-95,0,80],target:[5,22,-100],groundOffset:1.8},
  shore:{name:'Tidal beach',position:[-370,0,640],target:[-460,.4,760],groundOffset:1.6},
  surf:{name:'Wave close-up',position:[0,2,160],target:[0,0,160]},
...
export function createCamera(canvas,onKey=()=>{}) {
 ...
 const direction=b.forward.map((v,i)=>v*forward+b.right[i]*side+(i===1?up:0));const speed=(keys.has('ShiftLeft')||keys.has('ShiftRight')?180:25)*Math.min(dt,.05)/Math.max(1,Math.hypot(...direction));camera.x+=direction[0]*speed;camera.y=Math.max(-10,camera.y+direction[1]*speed);camera.z+=direction[2]*speed;return cameraBasis(camera);}};
}
