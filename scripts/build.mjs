...
for(const file of ['index.html','favicon.svg','LICENSE','src','generated','kernels'])await cp(path.join(root,file),path.join(dist,file),{recursive:true});
for(const file of ['vendor/coast/src/coast.js','vendor/realgrass/src','vendor/realgrass/vendor/cuda-webshader/src'])await cp(path.join(root,file),path.join(dist,file),{recursive:true});
