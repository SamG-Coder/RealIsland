...
test('all 26 precompiled CUDA entry points retain WGSL metadata',async()=>{const artifacts=JSON.parse(await text('generated/kernels.json'));assert.equal(Object.keys(artifacts).length,26);for(const a of Object.values(artifacts)){assert.ok(a.wgsl.includes('@compute'));assert.ok(a.metadata.bindings.length<=8);assert.equal(a.metadata.workgroupSize[1],1);}});
